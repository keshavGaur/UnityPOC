    ...
    void Awake()
    {
        /* GameObject g = GameObject.Find("Timer");
         if (SceneManager.GetActiveScene().name == "Level1")
         {
             if (g != null)
             {
                 g.GetComponent<WavesTimer>().timeBar = GameObject.Find("Sand").GetComponent<Image>();
                 //if (GameObject.Find("CurrentWave").GetComponent<Text>().ToString() != null || GameObject.Find("CurrentWave").GetComponent<Text>().ToString() != "") Debug.Log("ABLE TO FING CURRENTWAVE");
                 g.GetComponent<WavesTimer>().currentWaveText = GameObject.Find("CurrentWave").GetComponent<Text>();
                 GameObject.Find("TotalWaves").GetComponent<Text>().text = "6"; 
                 g.GetComponent<WavesTimer>().maxWaveNumberText = GameObject.Find("TotalWaves").GetComponent<Text>();
                 //Debug.Log(GameObject.Find("TotalWaves").GetComponent<Text>().text + " THIS IS TOTALWAVES TEXT");
                 g.GetComponent<WavesTimer>().highlightedFX = g.transform.Find("Highlight").gameObject;
                 g.GetComponent<WavesTimer>().highlightedTO = 0.5f;
             }
         }
         */
